...
export async function POST(request: Request) {
  try {
    ...
    for (const url of imageUrls) {
      ...
      const blob = await removeBackground(new Blob([inputBuffer.buffer.slice(inputBuffer.byteOffset, inputBuffer.byteOffset + inputBuffer.byteLength)], { type: "image/png" }));
      const id = nanoid();
